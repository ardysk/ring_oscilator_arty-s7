// Arty — 8× pierścień LUT + szeroki dzielnik (JA¹ L17 `ro_scope`) + drugie wyjście JA² (L18 `ro_scope_ring`) = zawsze
// zsynchronizowany przebieg z wybranego banku (bez programowego podziału, wyżej MHz na scope).
//
// BTN[1]/[0] — ±1 na `target_mhz` [1…511]; BTN[2]=meas_start; BTN[3]=reset; SW[0]=ro_en; SW[3]=1 wymusza bypass dzielnika na JA¹ (surowe `ro_out` przed próbkami div).
// • `F_RO_EST_MHZ_BANK` są typowymi punktami odniesienia — dostosuj po pomiarach (wpływa na dokładność ±1 MHz).
// • Przyciski i automatyczny wybór banku pracują w `clk_12mhz`. Wybrane **wyjściowe zbocza na JA pochodzą
//   z ROSC lub z licznika taktowanego ROSC**, więc to **„nowy”, asynchroniczny wobec SYSCLK zegar obserwowany”**.
// • Sterownik przycisków synchronizowany z SYSCLK sprawia że Układ jako całość **nie jest matematycznie
//   w 100 % pojedynczą domeną** — część kombinacji warunków żyje w SYSCLK część w zboczu pierścienia.
//
`timescale 1ns / 1ps
...
module ro_top_arty #(
    ...
    // Szerokość licznika dzielnika (domyślnie 16 bitów → bardzo niski podział przy wolnych targetach).
  ...
  // --- mapowanie banków (8 presetów tune) -----------------------------------------
  ...
  // Debounce (~12.5 ms @ 12 MHz) + impuls gdy przyciski stabilnie wg 1
