// Frequency counter: 1 ms gate @ clk_100mhz, RO async via 2-FF sync.
// f_hz = edges * F_REF_HZ / GATE_CYCLES  (GATE_CYCLES=100000 -> 1 ms).
`timescale 1ns / 1ps
...
module ro_freq_gate_100m #(
  ...
  // Gate enable in ref domain; also cross into RO domain (2-FF on ro_async clock)
