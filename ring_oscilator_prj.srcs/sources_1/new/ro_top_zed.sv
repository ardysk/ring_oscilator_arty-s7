// ZedBoard wrapper: DIP + BTNC + przyciski BTNU/R/L/D (strojenie MSB).
// Okno pomiaru: MEAS_GATE_CYCLES_DEFAULT — nie sprzęga się z DIP.
//
// Wielokrotny bank RO (równolegle + MUX na wyjściu rdzenia): domyślnie RO_BANKS=4.
//   sw[7:6] → ro_bank_sel (który fizyczny pierścień widać na ro_out / JA), pozostałe DIP na strojenie.
// Jeśli RO_BANKS<=1 → pełne 6 bitów DIP sw[7:2] jak wcześniej [tune].
//
// Strojenie (10 MUX-ów na każdy pierścień banku; kod powielany na RO_BANK przez {RO_BANKS{tune10}}).
// UWAGA: RO_BANKS=4 → DIP na strojeniu to sw[5:2]; RO_BANKS=2 → sw[6:2] + 1 buforowany 0 na LSB;
//         przy RO_BANKS<=1 jak dawniej pełny sw[7:2] w środkowych bitach pola strojenia.
//
`timescale 1ns / 1ps
